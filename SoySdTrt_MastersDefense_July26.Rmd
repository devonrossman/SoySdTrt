...
facet_grid(Year~location)+
  ...
  labs(x="Seed Treatment", y=expression(paste("Yield (Mg ", kg^-1, ")")), color="")
dev.off()
...
facet_grid(Year~location)+
  theme(text=element_text(color="black"), axis.text=element_text(size=8, angle=90, hjust=1),
    axis.title=element_text(size=10.5),
  strip.text=element_text(size=11, color="black"))+
  labs(x="Seed Treatment", y=expression(paste("Plant Stand (plants ",m^-2, ")" )), color="")
dev.off()
...
Y3$Var<-as.factor(Y3$Var)
lm1<-lmer(RDWdiffinsdtrt ~ sdtrt*Var*PathTitle + (1 | Rep/Exp), data=Y3)
...
geom_errorbar( aes(ymin=a$lsmean-1.96*a$SE, ymax=a$lsmean+1.96*a$SE, width=0.2), position=position_dodge(width=0.21))+
labs(y="Greenhouse Root Dry Weight \n Relative to NTC (g)", x="Seed Treatment", color="Seed Treament")+
...
Y3$var<-as.factor(Y3$var)
lm1<-lmer(RDWcont ~ sdtrt*var*location + (1 | rep/location), data=Y3)
...
grdw<-ggplot(a, aes(y=lsmean, x=sdtrt, color=sdtrt)
  ...
  geom_point(position=position_dodge(width=0.25))+
   geom_errorbar(aes( ymin=a$lsmean-1.96*a$SE, ymax=a$lsmean+1.96*a$SE), width=0.2, position=position_dodge(width=0.25))+
  labs(y="Root Dry Weight 2015 \n Relative to NTC (g)", x="Soybean Variety", color="Seed Treament")+
...
grdw<-ggplot(a, aes(y=lsmean, x=sdtrt, color=sdtrt)
  ...
  facet_grid(.~location)+
  geom_point(position=position_dodge(width=0.25))+
   geom_errorbar(aes( ymin=a$lsmean-1.96*a$SE, ymax=a$lsmean+1.96*a$SE), width=0.5, position=position_dodge(width=0.25))+
  labs(y="Plant Height 2014 \n Relative to NTC (cm)", x="Seed Treatment", color="Seed Treament")+
theme(text=element_text(size=9), axis.title=element_text(size=9), axis.text=element_text(size=8),
      axis.text.x=element_text(size=7.5, angle=90, hjust=1, vjust=0.5),
      legend.title=element_text(size=3), legend.text=element_text(size=2))
grdw
dev.off()
```