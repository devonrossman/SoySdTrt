...
D1 <-( ddply(Dglm, c("location", "sdtrt"), summarise,
            N=length(ScpubcontA),
             meanyield=mean(ScpubcontA),
            sdyield=sd(ScpubcontA),
            seyield=(sdyield/sqrt(N))
                           ))
limits<-aes(ymin=meanyield-(seyield*1.96), ymax=meanyield+(seyield*1.96))

jpeg("PredProbLocation.jpeg", width = 8, height = 5, units = 'in', res = 300)
...
facet_grid(.~location)+
  ...
  strip.text=element_text(size=11, color="black"), axis.text.y=element_text(hjust=0.5), axis.text.x=element_text(vjust=0.5))+
  labs(x="Seed Treatment", y=expression(paste("Plant Stand (plants ", A^-1, ")")), color="")+ scale_fill_manual(values = c("gray70", "gray50", "gray30", "black"))
dev.off()
