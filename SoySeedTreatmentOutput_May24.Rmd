...
g<-ggplot(a12, aes(y=lsmean, x=var, color=sdtrt)
  ...
  geom_point()+
  geom_errorbar(aes( ymin=(a12$lsmean-(1.96*a12$SE)), ymax=(a12$lsmean+(1.96*a12$SE)), width=0.2))+
  labs(y="Plant Stand Relative to NTC")
...
geom_errorbar(aes(ymin=a$lsmean-1.96*a$SE, ymax=a$lsmean+1.96*a$SE), width=0.2)+
  labs(y="Emergence Relative to NTC")
...
grdw<-ggplot(a12, aes(y=lsmeans.lsmean, x=lsmeans.var, color=lsmeans.sdtrt)
)+
  geom_point()+
...
a<-as.data.frame(a$lsmeans)
agh<-a
ghrdw<-ggplot(agh, aes(y=lsmean, x=Var, color=sdtrt)
)+ geom_point()+
geom_errorbar(aes( ymin=a$lsmean-1.96*a$SE, ymax=a$lsmean+1.96*a12$lsmeans.SE), width=0.2)+
labs(y="Greenhouse Root Dry Weight \n Relative to NTC")

multiplot(g, grdw, gh, ghrdw, cols=1)
